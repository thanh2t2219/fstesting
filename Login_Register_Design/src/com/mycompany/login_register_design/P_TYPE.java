...
public class P_TYPE {
    ...
    public HashMap<String, Integer> getTypesMap(){
        ...
        try {
            ...
            Logger.getLogger(P_TYPE.class.getName()).log(Level.SEVERE, name, ex);
        }
    ...
    public P_TYPE getTypeById(Integer id){
        ...
        try {
            ...
            ps = THE_CONNECTION.getTheConnection().prepareStatement("SELECT `id`, `name`, `description` FROM `property_type` WHERE `id`= ?");
            ps.setInt(1, id);
            rs = ps.executeQuery();
